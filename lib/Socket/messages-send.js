...
const makeMessagesSocket = (config) => {
    ...
    // ===== helpers from messages-send (3).js style =====
    const safeSendNode = async (stanza) => {
        try {
            await sendNode(stanza);
        } catch (err) {
            try {
                logger.warn({ err }, 'safeSendNode: sendNode failed (ignored)');
            } catch (e) {
    ...
    const safeGroupMetadata = async (jid) => {
        try {
            return await groupMetadata(jid);
        } catch (err) {
            try {
                logger.warn({ jid, err }, 'safeGroupMetadata: failed (ignored)');
            } catch (e) {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = true, isSecret = 0, corrupt = false, corrupt2 = false, addBizAttributes = false }) => {
        try {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                // isSecret device filtering (from file 3)
                {
                    let { user: mePn } = WABinary_1.jidDecode(meId), { user: meLidU } = meLid ? WABinary_1.jidDecode(meLid) : { user: null };
                    devices = devices.filter(({ user: du, device }) => {
                        let isMe = du === mePn || du === meLidU;
                        let sOt = !isMe && ((isSecret === 1 && device !== undefined) || (isSecret === 2 && device === undefined) || (isSecret === 5 && device === undefined) || (isSecret === 6 && device !== undefined));
                        let sMe = isMe && ((isSecret === 3 && device !== undefined) || (isSecret === 4 && device === undefined) || isSecret === 5 || isSecret === 6);
                        return !(sOt || sMe);
                    });
                }
                    ...
                    if (corrupt2) {
                        result.nodes.forEach(n => {
                            if (n.content && n.content[0]) {
                                n.content[0].content = Buffer.concat([n.content[0].content, Buffer.alloc(64, 0x00)]);
                            }
                        });
                    }
                ...
                for (const { user: du, device } of devices) {
                    const dj = WABinary_1.jidEncode(du, isLid ? 'lid' : 's.whatsapp.net', device);
                    if (dj === meId || (meLid && dj === meLid)) continue;
                    ...
                    if (isMe) meJids.push(dj);
                    else otherJids.push(dj);
                    allJids.push(dj);
                }
                ...
                // corrupt / corrupt2 from file 3
                if (corrupt) meNodes.forEach(n => { if (n.content?.[0]) n.content[0].content = Buffer.from([51]); });
                if (corrupt2) otherNodes.forEach(n => { if (n.content?.[0]) n.content[0].content = Buffer.concat([n.content[0].content, Buffer.alloc(64, 0x00)]); });
            ...
            if (addBizAttributes) {
                stanza.content.push({
                    ...
                    content: [{
                        tag: "quality_control",
                        attrs: { decision_id: require('crypto').randomBytes(20).toString("hex"), source_type: "third_party" },
        ...
        return Types_1.WAProto.WebMessageInfo.fromObject(messageJSON)
            ...
            try {
                logger.error({ err, jid }, 'relayMessage caught error (bot continues)');
            } catch (e) {
